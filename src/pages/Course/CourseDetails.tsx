...
const CourseDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      ...
      try {
        // Fetch basic details
        const detailsRes = await api.get<JSendResponse<Course>>(`/courses/${courseId}`);
        if (detailsRes.data.success && detailsRes.data.data) {
          setCourse(detailsRes.data.data);
        }

        // Fetch syllabus with progress
        const syllabusRes = await api.get<JSendResponse<SyllabusResponse>>(`/courses/${courseId}/syllabus`);
    ...
    return (
      <MainLayout>
        <div className="text-center py-12">
          ...
          <Link to="/courses">
            <Button variant="ghost" className="mt-4">Back to courses</Button>
          </Link>
        </div>
      </MainLayout>
    );
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                ...
                <span className="text-gray-500 text-sm">{course.meta.lessonCount} Lessons</span>
              </div>
            ...
            <div className="mt-4 md:mt-0 text-right">
              ...
              <div className="flex items-center space-x-3">
                ...
                <span className="font-bold text-gray-900">{syllabus.userProgress.percent}%</span>
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{lesson.orderIndex}. {lesson.title}</h3>
                      <p className="text-sm text-gray-500 capitalize">{lesson.type.toLowerCase()}</p>
                    </div>
                  </div>
                  <div>
                     {lesson.status !== 'LOCKED' ? (
                       <Link to={`/lessons/${lesson.id}`}>
                         <Button variant={lesson.status === 'COMPLETED' ? 'secondary' : 'primary'} size="sm">
