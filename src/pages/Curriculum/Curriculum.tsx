  ...
  const steps = [
    {
      ...
      courses: ['Web Fundamentals', 'JavaScript Basics', 'Git & Version Control'],
      status: 'COMPLETED'
    },
    {
      id: 2,
      ...
      courses: ['React Deep Dive', 'Advanced CSS & Tailwind', 'State Management'],
      status: 'IN_PROGRESS'
    },
    {
      id: 3,
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto">
        <div className="text-center py-10">
          <h1 className="text-4xl font-display font-bold text-gray-900 mb-4">Curriculum Roadmap</h1>
          <p className="text-gray-600">Your step-by-step guide to becoming a Senior Developer.</p>
        </div>
            ...
            <div key={step.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow 
                ${step.status === 'COMPLETED' ? 'bg-green-500 border-green-500' : 
                  step.status === 'IN_PROGRESS' ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-300'}`}>
                   {step.status === 'COMPLETED' ? (
                       <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                       </svg>
                   ) : (
                       <span className={`font-bold ${step.status === 'LOCKED' ? 'text-gray-400' : 'text-white'}`}>{index + 1}</span>
                   )}
              </div>
              
              {/* Card Content */}
              <Card className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 ${step.status === 'LOCKED' ? 'opacity-75 grayscale' : ''}`}>
                ...
                <div className="space-y-2">
                    {step.courses.map(course => (
                        <div key={course} className="flex items-center text-sm text-gray-700">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                    <Button variant="ghost" size="sm" className="w-full text-indigo-600" disabled={step.status === 'LOCKED'}>
