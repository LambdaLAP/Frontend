...
const MOCK_CONVERSATIONS: Conversation[] = [
  {
    ...
    messages: [
      { id: '1-1', role: 'user', content: 'How do I create a functional component?', timestamp: new Date() },
      { id: '1-2', role: 'assistant', content: 'You can create a functional component using a simple function that returns JSX. Here is an example...', timestamp: new Date() }
    ]
  },
  {
    id: '2',
    ...
    messages: [
      { id: '2-1', role: 'user', content: 'Can you explain async/await in JS?', timestamp: new Date() },
      { id: '2-2', role: 'assistant', content: 'Async/await is syntactic sugar on top of Promises...', timestamp: new Date() }
    ]
  }
];
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      const aiMessage: Message = {
        ...
        role: 'assistant',
        content: "I'm a mocked AI response. I see you said: \"" + inputValue + "\". Developing the real backend connection is our next step!",
        timestamp: new Date()
      };
      ...
      setConversations(prev => prev.map(c => {
        ...
        const targetId = currentChatId === NEW_CHAT_ID ? prev[0].id : currentChatId; // prev[0] is the one we just unshifted
  ...
  return (
    <MainLayout>
      <div className="flex h-[calc(100vh-64px)] bg-white overflow-hidden">
        {/* Sidebar */}
        <div className="w-64 bg-gray-900 flex-shrink-0 flex flex-col hidden md:flex">
          <div className="p-4">
            <button 
              onClick={handleNewChat}
              className="w-full flex items-center gap-3 px-4 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-md transition-colors border border-gray-700 text-sm mb-4"
            >
          ...
          <div className="p-4 border-t border-gray-800">
             <div className="flex items-center gap-3 px-2 py-2 text-gray-300 text-sm hover:bg-gray-800 rounded cursor-pointer">
                <div className="w-8 h-8 rounded bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold">
                    L
                </div>
                <div>My Account</div>
             </div>
        ...
        <div className="flex-1 flex flex-col relative w-full">
                                ...
                                <div className="max-w-3xl mx-auto px-4 py-8 flex gap-4 md:gap-6">
                                    <div className="flex-shrink-0 flex flex-col relative items-end">
                                        <div className={`w-8 h-8 rounded-sm flex items-center justify-center ${
                                            msg.role === 'assistant' ? 'bg-green-500' : 'bg-indigo-600'
                                        }`}>
                                            {msg.role === 'assistant' ? (
            ...
            <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-white via-white to-transparent pt-10 pb-6 px-4">
                <div className="max-w-3xl mx-auto relative">
                    <form onSubmit={handleSendMessage} className="relative flex items-center w-full p-3 bg-white border border-gray-200 rounded-xl shadow-lg focus-within:ring-1 focus-within:ring-black/10">
                        <input 
                        ...
                        <button 
                            ...
                            className={`p-2 rounded-md transition-colors ${
                                inputValue.trim() ? 'bg-indigo-500 text-white' : 'bg-transparent text-gray-300'
                            }`}
                        >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
